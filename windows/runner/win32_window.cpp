...
Win32Window::MessageHandler(HWND hwnd,
                            ...
                            LPARAM const lparam) noexcept {
  switch (message) {
    // Windows is shutting down / logging off, or the auto-update installer is
    // closing us through the Restart Manager. NEVER prompt here: a silent
    // background update would hang forever on a dialog nobody is watching.
    // Remember it so the WM_CLOSE that follows goes straight through.
    case WM_QUERYENDSESSION:
      closing_without_prompt_ = true;
      return TRUE;

    // The user clicked X or pressed Alt+F4. Confirm, so a mis-click doesn't
    ...
    case WM_CLOSE: {
      ...
      closing_without_prompt_ = true;  // don't ask again on the way out
      break;
    }
